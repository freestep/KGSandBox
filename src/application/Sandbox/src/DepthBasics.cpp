...
void CDepthBasics::ProcessDepth(INT64 nTime, const UINT16* pBuffer, int nWidth, int nHeight, USHORT nMinDepth, USHORT nMaxDepth)
{
    ...
    if (m_pDepthRGBX && pBuffer && (nWidth == cDepthWidth) && (nHeight == cDepthHeight))
    {
				...
				for (int j = 0; j < nWidth; j++)
				{
					for (int k = -p; k < p; k++)
					{
						for (int l = -p; l <= p; l++)
						{
							m = i + k; n = j + l;
							if (m < 0) m = abs(i + k) - 1;
							if (m > nHeight - 1) { m = 2 * nHeight - i - k - 1; }
							if (n < 0) n = abs(j + l) - 1;
							if (n > nWidth - 1) { n = 2 * nWidth - j - l - 1; }
							aa1 = exp(-a1*(l*l + k*k));
		...
		const UINT16* lpdst = *f;
		const UINT16* pBufferEnd = useBilateralFilter ? (lpdst + (nWidth * nHeight)) : (pBuffer + (nWidth * nHeight));
		...
		{
#if  1
			while ((useBilateralFilter ? lpdst : pBuffer) < pBufferEnd)
			{
				USHORT depth = useBilateralFilter ? *lpdst : *pBuffer;
				...
				++pRGBX;
				if (useBilateralFilter) ++lpdst;
				else ++pBuffer;
				//useBilateralFilter ? (++lpdst) : (++pBuffer);
			}
#endif
		}
